...
body.onresize = function () {
  const hist = document.querySelector("#histogram")
  if (hist != null) {
    hist.remove()
    drawHist()
  }
  const word_cloud = document.querySelector('#wordcloud')
  if (word_cloud != null) {
    word_cloud.remove()
    drawWordCloud()
  }
...
function drawHist() {
  ...
  // get the data
  d3.csv("data/txt_analysis_word_length.csv", function (data) {
...
function drawWordCloud() {
  ...
  // get the data
  d3.csv("data/text_analysis_comments.csv", function (data) {
  //d3.csv("data/text_analysis_comments.csv").then(function (data) {
  ...
  var svg = d3.select("#wordcloud").append("svg")
    ...
    .attr("transform",
      ...
      var layout = d3.layout.cloud()
        .size([width, height])
        .words(data.map(function (d) { return { text: d.word, size: d.freq }; }))
        .padding(5)        //space between words
        .rotate(function () { return ~~(Math.random() * 2) * 90; })
        .fontSize(function (d) { return d.freq; })      // font size of words
        .on("end", draw);
      layout.start();
      ...
      function draw(words) {
        svg
          ...
          .data(words)
          .enter().append("text")
          .style("font-size", function (d) { return d.freq; })
          .style("fill", "#69b3a2")
          .attr("text-anchor", "middle")
